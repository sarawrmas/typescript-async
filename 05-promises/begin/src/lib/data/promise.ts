...
const getHeroPromise = (email: string) => {
  return axios.get<Hero[]>(`${apiUrl}/heroes?email=${email}`)
...
const getOrdersPromise = function(heroId: number) {
  return axios.get<Order[]>(`${apiUrl}/orders/${heroId}`)
  .then((response: AxiosResponse<Order[]>) => 
    parseList<Order>(response)
  )
  .catch((error: AxiosError) => handleAxiosErrors(error, 'Order'));
};
...
const getAccountRepPromise = function(heroId: number) {
  return axios.get<AccountRepresentative[]>(`${apiUrl}/accountreps/${heroId}`)
